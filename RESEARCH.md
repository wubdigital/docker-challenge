...
1. Faster Push/Pull Operations & Minimal Deployment Time
...
2. Enhanced Security (Reduced Attack Surface)
...
3. Reduced Storage & Infrastructure Costs
...
4. Healthcheck - תלות תקינה בין קונטיינרים
...
# 1. Copying package files and modifying ownership to the unprivileged user

COPY --chown=node:node package\*.json ./

# 2. Installing only runtime production dependencies

RUN npm install --only=production

# 3. Copying application source code and modifying ownership
...
3. Environment Separation (Development vs. Production)
4. The Multi-File Merge Mechanism
...
5. Dual Runtime Environments Configuration
...
3. Dual Runtime Execution Commands
   ...
   To initiate the merged configuration (Base + Override) with Hot Reload active, execute:
   docker compose up
   ...
   Production Mode
   To enforce a clean deployment while completely isolating the runtime from local development configs and tools, execute: docker compose -f docker-compose.yml up --build
...
3. Implementing Nginx as a Reverse Proxy
4. Conceptual Breakdown: What is a Reverse Proxy?
   ...
   🛠️ 2. Production Architecture Implementation
